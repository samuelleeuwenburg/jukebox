...
module Conn = {
  let handle = (_io, socket, _getState, _dispatch) => {
    socket->SocketIO.on(Types.Socket.RequestAccessToken, refreshToken => {
      withClientSecret(clientSecret => {
        ...
        |> Js.Promise.then_(json => {
          ...
          socket->SocketIO.emit2(Types.Socket.SendAccessToken, token, expiresIn)
    ...
    socket->SocketIO.on2(Types.Socket.RequestRefreshToken, (. clientURI, code) => {
      open Js.Promise
      withClientSecret(clientSecret => {
